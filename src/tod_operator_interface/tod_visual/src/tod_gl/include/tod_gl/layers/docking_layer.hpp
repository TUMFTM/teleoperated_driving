/**
 * @file docking_layer.hpp
 * @brief Base layer for manageing the dockerspace for the application's docking layer such as dock events and direction 
...
class DockingLayer : public ImGuiLayer {
  public:
    DockingLayer(std::shared_ptr<RosInterface> ros, ImGuiDir split_dir);
  ...
  private:
    ...
    std::vector<DockingLayer*> _layers_to_dock;
};
