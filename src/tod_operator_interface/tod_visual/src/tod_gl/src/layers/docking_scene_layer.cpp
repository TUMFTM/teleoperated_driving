...
DockingSceneLayer::DockingSceneLayer(std::shared_ptr<RosInterface> ros, std::shared_ptr<Scene> scene, ImGuiDir split_dir)
    : ImGuiSceneLayer(ros, scene) {
    _name = "DockingSceneLayer";
    split_direction = split_dir;
}

void DockingSceneLayer::on_attach() {
    should_update_layout = true;
}

void DockingSceneLayer::on_detach() {
    should_update_layout = true;
}

void DockingSceneLayer::on_im_gui_render() {
...
void DockingSceneLayer::on_event(Event& e) {
    ...
    SceneLayerStackChangedEvent event = static_cast<SceneLayerStackChangedEvent&>(e);

    auto* layer = dynamic_cast<DockingSceneLayer*>(event.get_layer());
    if (!layer || layer == this)
        return;
...
void DockingSceneLayer::update_layout() {
    ImGuiID dockspace_id = ImGui::GetID(_dock_space_name.c_str());
    ...
    for (DockingSceneLayer* layer : _layers_to_dock) {
