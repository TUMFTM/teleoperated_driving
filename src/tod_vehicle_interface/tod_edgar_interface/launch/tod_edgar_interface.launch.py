...
def generate_launch_description():
    ...
    config_path_arg = DeclareLaunchArgument(
        ...
        default_value=os.path.join(
            get_package_share_directory('tod_edgar_interface'),
            'config'
        ),
    ...
    package_dir = get_package_share_directory('tod_edgar_interface')

    # Sub-launches (pass down the config_path explicitly as an argument)
    actuation_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(package_dir, 'launch', 'edgar_actuation_interface.launch.py')),
        launch_arguments={'config_path': config_path}.items()
    )

    sensing_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource(os.path.join(package_dir, 'launch', 'edgar_sensing_interface.launch.py')),
        launch_arguments={'config_path': config_path}.items()
    )
