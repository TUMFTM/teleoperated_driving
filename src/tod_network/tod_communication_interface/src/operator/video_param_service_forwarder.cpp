/**
 * @file video_param_service_forwarder.cpp
...
int main(int argc, char** argv) {
    ...
    if (argc > 5) {
        RCLCPP_DEBUG(rclcpp::get_logger("VideoParamServiceForwarder"), "Warning: Extra arguments ignored.\n");
    }

    std::shared_ptr<tod_network::ServiceForwarder<rcl_interfaces::srv::SetParameters>> network_monitor_service_forwarder;

    boost::asio::io_context io;

    if (protocol == "UDP") { // UDP
        network_monitor_service_forwarder = std::make_shared<tod_network::ServiceForwarder<rcl_interfaces::srv::SetParameters>>(
