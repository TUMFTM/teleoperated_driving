/**
 * @file video_config_service_forwarder.cpp
...
int main(int argc, char** argv) {
    ...
    if (argc > 5) {
        RCLCPP_DEBUG(rclcpp::get_logger("VideoConfigServiceForwarder"), "Warning: Extra arguments ignored.\n");
    }

    std::shared_ptr<tod_network::ServiceForwarder<tod_config_msgs::srv::VideoConfig>> video_config_service_forwarder;

    boost::asio::io_context io;

    if (protocol == "UDP") {  // UDP
        video_config_service_forwarder = std::make_shared<tod_network::ServiceForwarder<tod_config_msgs::srv::VideoConfig>>(
    ...
    rclcpp::spin(video_config_service_forwarder);
    rclcpp::shutdown();
