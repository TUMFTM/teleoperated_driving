/**
 * @file network_monitor_service_forwarder.cpp
...
int main(int argc, char** argv) {
    ...
    if (argc > 5) {
        RCLCPP_DEBUG(rclcpp::get_logger("NetworkMonitorServiceListener"), "Warning: Extra arguments ignored.\n");
    }

    std::shared_ptr<tod_network::ServiceForwarder<tod_network_monitoring_msgs::srv::NetworkMonitorService>> network_monitor_service_forwarder;

    boost::asio::io_context io;

    if (protocol == "UDP") {  // UDP
        network_monitor_service_forwarder = std::make_shared<tod_network::ServiceForwarder<tod_network_monitoring_msgs::srv::NetworkMonitorService>>(
    ...
    rclcpp::spin(network_monitor_service_forwarder);
    rclcpp::shutdown();
