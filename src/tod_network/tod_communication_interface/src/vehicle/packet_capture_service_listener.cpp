/**
 * @file packet_capture_service_listener.cpp
...
int main(int argc, char** argv) {
    ...
    if (argc > 5) {
        RCLCPP_DEBUG(rclcpp::get_logger("PacketCaptureServiceListener"), "Warning: Extra arguments ignored.\n");
    }

    std::shared_ptr<tod_network::ServiceListener<tod_network_monitoring_msgs::srv::PacketCaptureService>> packet_capture_service_listener;

    boost::asio::io_context io;

    if (protocol == "UDP") {  // UDP
        packet_capture_service_listener = std::make_shared<tod_network::ServiceListener<tod_network_monitoring_msgs::srv::PacketCaptureService>>(
    ...
    rclcpp::spin(packet_capture_service_listener);
    rclcpp::shutdown();
