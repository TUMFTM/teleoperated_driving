/**
 * @file video_config_service_listener.cpp
...
int main(int argc, char** argv) {
    ...
    if (argc > 5) {
        RCLCPP_DEBUG(rclcpp::get_logger("VideoConfigServiceListener"), "Warning: Extra arguments ignored.\n");
    }

    std::shared_ptr<tod_network::ServiceListener<tod_config_msgs::srv::VideoConfig>> video_config_service_listener;

    boost::asio::io_context io;

    if (protocol == "UDP") {  // UDP
        video_config_service_listener = std::make_shared<tod_network::ServiceListener<tod_config_msgs::srv::VideoConfig>>(
    ...
    rclcpp::spin(video_config_service_listener);
    rclcpp::shutdown();
